...
class Gdn_PasswordHash extends PasswordHash {
    ...
    * @return boolean
    */
   //FACTODO START : ajout du crypt avec le salt
   function CheckPassword($Password, $StoredHash, $Method = FALSE, $Salt, $Username = NULL) {
      $Result = FALSE;
		switch(strtolower($Method)) {
         case 'django':
         ...
         case 'punbb':
            ...
            break;
         case 'dogma':
           if(crypt($Password,$Salt) == $StoredHash || crypt($Password, $StoredHash) == $StoredHash)
             $Result =  TRUE;
           else
           {
             $Result = FALSE;
