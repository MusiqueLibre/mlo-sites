...
<?php
    ...
    public function get_github_commits($user, $repo, $count = 5) {
        ...
        foreach($issues as $issue) {
            ...
            $output .= '<li class = "github-issue sidebar_list bullet_less">';
            $output .= ' &#10003; <s><a href="'.$issue->html_url.'">'.$issue->title.'</a></s>';
            $output .= '<br/><span class="sidebar-meta"> '.__('closed on', 'wp-github-commits') . ' ' . date("d/m/y", strtotime($issue->closed_at)).'</span>';
            $output .= '</li>';
            if ($count == $counter) {
                break;
            }
        ...
        $output .= '</ul>';
        $output .= '<h3>'.__('Latest updates', 'wp-github-commits').' : '.'</h3>';
        $output .= '<ul class = "github-commits bullet_less_list">';
        $counter = 0;
        foreach($commits as $commit) {
            $counter ++;
            $output .= '<li class = "github-commit sidebar_list bullet_less">';
            $output .= "&#8657; <a href = 'https://github.com/$user/$repo/commit/{$commit->sha}'>" . $commit->commit->message . '</a> ';
            $output .= '<br/><span class="sidebar-meta">'.__('on', 'wp-github-commits') . ' ' . date("d/m/y", strtotime($commit->commit->author->date)).'</span>';
            $output .= '</li>';
